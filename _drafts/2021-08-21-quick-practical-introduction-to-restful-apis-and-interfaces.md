...
that it's easy to use and support such inferfaces.
...
to the websites but the APIs themselves. There were a few traits of web-based systems:
 ...
 - The web is staleless (at least the most of the early web)
 - The responses are cachable
...
will be a fundemental technique to build distributed APIs on the web.
...
These constrains (guidelines) are (were):
...
## Use HTTP protocol
HTTP protocol is simple and well-understood. There is a lot of infrastructure to faciliate 
HTTP-based APIs (client and server libraries) so it's easy to use it.

Almost all programming languages support HTTP protocal out-of-the-box which would't
...
First of all, what is a resource. The easiest way to describe it, it would be to say
...
## Use HTTP methods to faciliate different operations
Now we know where is our stuff is stored, so we can start doing something with it.

For that, we use the folllowing HTTP verbs
...
*POST* - to create new content
  - `POST /users/` @CONTENT - create a new users with the details that have been specified in the @CONTENT payload.
...
When doing the post requests, there are a few ways to handle the responses:
 ...
 2. Return "Location" header and redirect the user to the newly created resource
...
your exploring API and can provide additional context of what could be done 
with the entity.

There is, so called, [Maturity Model](https://martinfowler.com/articles/richardsonMaturityModel.html) for APIs:
 - Level 0 - no RESTful aspects on the API
 - Level 1 - API has a concept of resources
 - Level 2 - HTTP Verbs are used to faciliate domain logic
...
and they are consumed by internal teams.
...
I can get a list of order for the user?" is often good enough solution.
...
JSON is as widely supported as HTTP itself and you will have less problems with your clients.
Also, it's easy for humans to read an understand (definely better than ProtoBuf).

XML used to be an option 10-15 years ago but these days it's too verbose and XML parsing libraries
are quite heavy (dependency wise) and slugish.
...
Also, please note that if we introduce a new version of orders via URL, it doesn't mean that the other versions need to be moved/bumped two. There could be multiple version active at the same
time:
 - `/api/v1/orders/`
 - `/api/v2/orders/`
 - `/api/v1/users/`
...
will have to retrieve the entire document from the database. And then, there is
 very little extra cost of sending a few more kB over the wire.
...
Normally, there should be no problems to handle 1-4MB response sizes
for your HTTP client. In 4MB you can fit in 1000 entries that are 4kB big. And 4kB is a lot 
of text. Compressing that with gzip (and you should enable that on your web server), 
...
In the end, premature optimization is root of all evil. Optimize your payloads when
...
What's BAD:
 ...
 - Renaming/Removing existing query paramaters

# Pitfalls to avoid
There are some pitfalls that are easy to notice and they should be avoided.
...
Even HTTP clients will be confused as you will be getting erros with GET requests:
...
So if you order is in "pending" status, and the client sends a request like this:
```
{
  "status": "handling"
}
...
Business process logic should be the domain logic and it shouln't be part of the RESTful interface.
...
Just having JSON is enough in 99% of occasions and if by pure chance somebody will start
using XML version of the API, you will have to maintain that.
...
HTTP Basic Auth should be in format: `user_id:api_key` . This way you will support all of the HTTP
client out-of-the-box and it will be super simple to use.
...
If every request reponds with [HTTP status](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes)
 code 200 or 500 there might be missed opportunities to utilize HTTP stack better.
...
# Resources
 - The initial dissertation [https://www.ics.uci.edu/~fielding/pubs/dissertation/rest_arch_style.htm]
